...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Calculate coin age reward
    {
        ...
        bool hittinglimit = PoSRewardHittingLimit(nCoinAge, pindexBest);
        if (nReward <= 0 && hittinglimit == false)
        {
            return false;
        }
...
{
    {
        ...
        if (block.IsProofOfWork() && mapWallet.count(block.vtx[0].GetHash()))
        {
            ...
            printf("    mine:  %d  %d  %" PRIx64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
        }
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            CWalletTx& wtx = mapWallet[block.vtx[1].GetHash()];
            printf("    stake: %d  %d  %" PRIx64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
         }
...
{
    {
        ...
        printf("CWallet::NewKeyPool wrote %" PRIx64 " new keys\n", nKeys);
    }
    return true;
}
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            printf("keypool added key %" PRIx64 ", size=%" PRIszu"\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
